...
for i in range(len(X_test_cont) - ngram):
    Xtest.append(X_test_cont[i:i+ngram])
    Ytest.append(X_test_cont[i+ngram])
    
Xtest = np.array(Xtest)
Ytest = np_utils.to_categorical(Ytest)
top_words = Ytest.shape[1]
...
model.fit_generator(generate_data(), samples_per_epoch = len(Xdata), nb_epoch=10, verbose=2)
